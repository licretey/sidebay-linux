...
- 日期：2026-08-02
- 作者：架构师审视角
- 目标：**功能不变的前提下**——精简复用实现、降低内存（本地 ≤75MB、flatpak ≤125MB，基于实测基线修订）、降低 CPU 占用
...
| 指标 | 当前 | 目标（修订） |
|---|---|---|
| 本地 RSS | 91MB | **≤75MB**（2026-08-02 实测基线：python+gi=54MB、空窗口=58MB——60MB 在 Python+GTK4 下不可达，硬下限 ~58MB） |
| Flatpak RSS | 144MB | **≤125MB**（沙盒固有开销 ~50MB 不可压缩；100MB 需应用 ≤40MB，超出现有技术栈物理下限） |
| 若要 60MB/100MB | — | 需 Rust/GTK-rs 或 Tauri 重写（另立项目） |
| 进程 timer 数 | 5+1 | 2+1 |
...
| S5 | P2 测量脚本 + 回归 | 本地 ≤75MB、flatpak ≤125MB |
